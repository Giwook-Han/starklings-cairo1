...
mod LizInventory {
    ...
    fn add_stock(product: felt252, new_stock: u32) {
        ...
        assert(get_caller_address() == contract_owner::read() , 'only owner can call this');
        inventory::write(product,inventory::read(product) + new_stock);
    }
    ...
    fn purchase(product: felt252, quantity: u32) {
        ...
        assert(inventory::read(product) > quantity, 'quantity is bigger than stock');
        
        inventory::write(product,inventory::read(product) - quantity);
    }
